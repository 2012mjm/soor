...
import { attributeThunk } from '../../../thunks/attribute'
...
class AutoCompletor extends React.Component {
  ...
  componentDidMount () {
    ...
    this.props.dispatch(attributeThunk()).then((res) => this.setState({
  ...
  search (keyword) {
    ...
    if (query !== '') {
      ...
      if(result.length === 0) result = [{text: 'خواصی یافت نشد.', value: null}]
    }
    else {
